// Daniel Harvey
// combining Adafruit_TSL2561.cpp (by K. Townsend) and Adafruit_HTU21DF.cpp (by Limor Fried/Ladyada) using SoftI2CMaster.h (by Tod E. Kurt) instead of _i2c.h
...
Adafruit_HTU21DF::Adafruit_HTU21DF(uint8_t sda, uint8_t scl) {
	_i2c = SoftI2CMaster(scl, sda);
}
...
boolean Adafruit_HTU21DF::begin(void) {
  ...
  _i2c.beginTransmission(HTU21DF_I2CADDR);
  _i2c.write(HTU21DF_READREG);
  _i2c.endTransmission();
  _i2c.requestFrom(HTU21DF_I2CADDR);
  return (_i2c.readLast() == 0x2); // after reset should be 0x2
}

void Adafruit_HTU21DF::reset(void) {
  _i2c.beginTransmission(HTU21DF_I2CADDR);
  _i2c.write(HTU21DF_RESET);
  _i2c.endTransmission();
  delay(15);
}
...
float Adafruit_HTU21DF::readTemperature(void) {
  ...
  uint16_t t = _i2c.read();
  t <<= 8;
  t |= _i2c.read();

  uint8_t crc = _i2c.readLast();
...
Adafruit_TSL2561::Adafruit_TSL2561(uint8_t sda, uint8_t scl, uint8_t addr, int32_t sensorID) 
{
  _i2c = SoftI2CMaster(scl, sda);
  _addr = addr;
